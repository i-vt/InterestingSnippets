# Bytes to exclude
to_exclude = [b'\x02', b'\x03']

# Generate badchars with exclusions
badchars = generate_badchars(to_exclude)

def generate_badchars(exclude=None):
    if exclude is None:
        exclude = []
    return bytes(b for b in range(1, 256) if bytes([b]) not in exclude)

for b in generate_badchars(to_exclude):
    print(f"b\"\\x{b:02x}\"")

"""
No need to use this list anymore lol:
badchars = (
